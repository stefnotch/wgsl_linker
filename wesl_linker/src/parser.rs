...
use crate::SyntaxKind as Token;
use crate::{Parse, ParseEntryPoint, lexer::tokenize};
...
pub fn parse_entrypoint(input: &str, entrypoint: ParseEntryPoint) -> Parse {
    ...
    let parser = Parser::new_with_context(input, &mut diagnostics, ParserContext {});
...
impl Cst<'_> {
    ...
    pub fn get_text(&self, index: CstIndex) -> &str {
        &self.source[self.get_span(index)]
    }

    pub fn get_span(&self, index: CstIndex) -> std::ops::Range<usize> {
...
impl<'source> ParserCallbacks<'source> for Parser<'source> {
    ...
    fn create_node_if_statement(&mut self, node_ref: NodeRef, diags: &mut Vec<Self::Diagnostic>) {
